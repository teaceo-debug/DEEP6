// DEEP6 Footprint V8 — NinjaTrader 8 indicator.
//
// This file contains the DEEP6 Footprint V8 indicator: a simplified, low-noise
...
//
// This file contains the main DEEP6 Footprint V8 indicator: the FootprintBar / Cell
...
//   %USERPROFILE%\Documents\NinjaTrader 8\bin\Custom\Indicators\DEEP6\DEEP6FootprintV8.cs
...
{
    public class DEEP6FootprintV8 : Indicator
    {
        // ---- State ----
        ...
        private readonly Queue<int> _recentSignalDirections = new Queue<int>();
        ...
        private readonly ExhaustionDetector _rtExhDetector = new ExhaustionDetector();
        ...
        // Part C: frequency limiter — reset each session
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description         = "DEEP6 Footprint V8 — low-noise execution footprint with plain-English setup, ready, and triggered signals.";
                Name                = "DEEP6 Footprint V8";
                ...
                // ── Part A: granular per-variant toggles (V8 audit-driven defaults) ──
                ShowClassicAbsorption   = true;    // ABS_01 — optimized ON (iter 67)
                ShowPassiveAbsorption   = false;   // ABS_02 — redundant with ABS_01 per T7
                ShowStoppingVolume      = false;   // ABS_03 — INCONCLUSIVE
                ShowEffortVsResult      = true;    // ABS_04 — optimized ON (iter 67)
                ShowZeroPrint           = false;   // EXH_01 — dominated by EXH_02
                ShowExhaustionPrint     = true;    // EXH_02 — best exhaustion variant
                ShowThinPrint           = false;   // EXH_03 — dominated by EXH_02
                ShowFatPrint            = true;    // EXH_04 — optimized ON (iter 67)
                ShowFadingMomentum      = false;   // EXH_05 — dominated by EXH_02
                ShowBidAskFade          = true;    // EXH_06 — KEEP per T7

                // ── Part B: arrow gating thresholds ──
                MinArrowConfluence      = 4;
                MinExhaustionStrength   = 0.504331;
                BiasLongThreshold       = 0.59481;
                BiasShortThreshold      = -0.649336;
                BiasLookback            = 4;
                ShowTriggeredArrows     = true;

                // ── Part C: signal frequency limiter ──
            ...
            else if (State == State.DataLoaded)
            {
                ...
                _rtExhDetector.ResetCooldowns();
                _atrWindow.Clear();
                _recentSignalDirections.Clear();
                _volEma = 0.0;
                _priorCvd = 0;
        ...
        private int _lastRtAbsCount;
        private int _lastRtExhCount;

        private void RunRealtimeDetection()
        {
            FootprintBar snapshot;
            lock (_barsLock)
            {
                FootprintBar live;
                if (!_bars.TryGetValue(CurrentBar, out live)) return;
                // Clone to avoid race with OnMarketData
                snapshot = new FootprintBar { BarIndex = live.BarIndex };
                foreach (var kvp in live.Levels)
                    snapshot.Levels[kvp.Key] = new Cell { BidVol = kvp.Value.BidVol, AskVol = kvp.Value.AskVol, NeutralVol = kvp.Value.NeutralVol };
                snapshot.Open = live.Open; snapshot.High = live.High;
                snapshot.Low = live.Low; snapshot.Close = live.Close;
                snapshot.TotalVol = live.TotalVol; snapshot.RunningDelta = live.RunningDelta;
                snapshot.MaxDelta = live.MaxDelta; snapshot.MinDelta = live.MinDelta;
            }
            snapshot.Finalize(_priorCvd);
            if (snapshot.TotalVol < 20) return;

            // Clear previous RT drawings
            for (int i = 0; i < Math.Max(_lastRtAbsCount, 10); i++)
                RemoveDrawObject("RT_Abs_" + CurrentBar + "_" + i);
            for (int i = 0; i < Math.Max(_lastRtExhCount, 10); i++)
                RemoveDrawObject("RT_Exh_" + CurrentBar + "_" + i);
            RemoveDrawObject("RT_BiasBox_" + CurrentBar);
            _lastRtAbsCount = 0;
            _lastRtExhCount = 0;

            var va = FootprintBar.ComputeValueArea(snapshot, TickSize);

            // RT Absorption
            if (ShowAbsorptionMarkers)
            {
                var abs = AbsorptionDetector.Detect(snapshot, _atr, _volEma, _absCfg, va.vah, va.val, TickSize);
                _lastRtAbsCount = abs.Count;
                for (int i = 0; i < abs.Count; i++)
                {
                    var s = abs[i];
                    switch (s.Kind)
                    {
                        case AbsorptionType.Classic:        if (!ShowClassicAbsorption) continue; break;
                        case AbsorptionType.Passive:        if (!ShowPassiveAbsorption) continue; break;
                        case AbsorptionType.StoppingVolume: if (!ShowStoppingVolume)    continue; break;
                        case AbsorptionType.EffortVsResult: if (!ShowEffortVsResult)    continue; break;
                    }
                    // RT absorption: diamond only, no text
                    string tag = "RT_Abs_" + CurrentBar + "_" + i;
                    Brush brush = s.Direction >= 0 ? Brushes.LimeGreen : Brushes.Crimson;
                    double price = s.Direction >= 0 
                        ? s.Price - 2 * TickSize
                        : s.Price + 2 * TickSize;
                    Draw.Diamond(this, tag, false, 0, price, brush);
                }
            }

            // RT Exhaustion — reset cooldowns so signals don't disappear mid-bar
            _rtExhDetector.ResetCooldowns();
            if (ShowExhaustionMarkers)
            {
                var exh = _rtExhDetector.Detect(snapshot, _priorFinalized, CurrentBar, _atr, _exhCfg);
                _lastRtExhCount = exh.Count;
                for (int i = 0; i < exh.Count; i++)
                {
                    var s = exh[i];
                    switch (s.Kind)
                    {
                        case ExhaustionType.ZeroPrint:        if (!ShowZeroPrint)        continue; break;
                        case ExhaustionType.ExhaustionPrint:   if (!ShowExhaustionPrint)  continue; break;
                        case ExhaustionType.ThinPrint:         if (!ShowThinPrint)        continue; break;
                        case ExhaustionType.FatPrint:          if (!ShowFatPrint)          continue; break;
                        case ExhaustionType.FadingMomentum:    if (!ShowFadingMomentum)   continue; break;
                        case ExhaustionType.BidAskFade:        if (!ShowBidAskFade)       continue; break;
                    }
                    if (s.Strength < MinExhaustionStrength) continue;

                    // RT exhaustion: triangle only
                    string tag = "RT_Exh_" + CurrentBar + "_" + i;
                    if (s.Direction > 0)
                        Draw.TriangleUp(this, tag, false, 0, s.Price - 4 * TickSize, Brushes.LimeGreen);
                    else if (s.Direction < 0)
                        Draw.TriangleDown(this, tag, false, 0, s.Price + 4 * TickSize, Brushes.Crimson);
                }
            }
            ...
            if (ShowBiasBox)
            {
                double bias = ComputeBiasScore(BiasLookback);
                string txt = bias > BiasLongThreshold ? "LONG" : bias < BiasShortThreshold ? "SHORT" : "-";
                Brush col = bias > BiasLongThreshold ? Brushes.LimeGreen :
                            bias < BiasShortThreshold ? Brushes.Crimson : Brushes.DimGray;
                Draw.Text(this, "RT_BiasBox_" + CurrentBar, false, txt, 0, Close[0], 8,
                    Brushes.White, new SimpleFont("Arial", 9) { Bold = true },
                    System.Windows.TextAlignment.Center, col, col, 80);
            }
        }
        ...
        protected override void OnBarUpdate()
        {
            ...
            // Real-time detection on current forming bar (every tick)
            ...
            // Clean up RT drawings from previous bar
            for (int i = 0; i < 20; i++)
            {
                RemoveDrawObject("RT_Abs_" + prevIdx + "_" + i);
                RemoveDrawObject("RT_Exh_" + prevIdx + "_" + i);
            }
            RemoveDrawObject("RT_BiasBox_" + prevIdx);
            ...
            if (isNewSessionBar)
            {
                ...
                _sessionBarCount = 0;
                _sessionSignalCount = 0;  // Part C: reset frequency limiter
        ...
        {
            // Part B: triggered arrow toggle
            if (!ShowTriggeredArrows) return;
            // Part C: frequency limiter
            if (MaxSignalsPerSession > 0 && _sessionSignalCount >= MaxSignalsPerSession) return;
            ...
            if (isLong)
                Draw.ArrowUp(this, "V8_TRIGGER_" + suffix, false, barsAgo, price - 6.0 * TickSize, pick);
            else
                Draw.ArrowDown(this, "V8_TRIGGER_" + suffix, false, barsAgo, price + 6.0 * TickSize, pick);

            Draw.Text(this, "V8_TRIGGER_LBL_" + suffix, isLong ? "Long Triggered" : "Short Triggered",
        ...
        private void TrackRecentSignalDirection(int direction)
        {
            if (direction == 0) return;

            _recentSignalDirections.Enqueue(direction > 0 ? 1 : -1);
            int maxHistory = System.Math.Max(3, System.Math.Min(10, BiasLookback));
            while (_recentSignalDirections.Count > maxHistory)
                _recentSignalDirections.Dequeue();
        }

        private double ComputeBiasScore(int barsBack = 3)
        {
            int lookback = System.Math.Max(1, System.Math.Min(10, barsBack));
            if (_recentSignalDirections.Count == 0)
                return 0.0;

            int[] snapshot = _recentSignalDirections.ToArray();
            int start = System.Math.Max(0, snapshot.Length - lookback);
        ...
        {
            // Part A: per-variant toggle gate
            switch (s.Kind)
            {
                case AbsorptionType.Classic:        if (!ShowClassicAbsorption)  return; break;
                case AbsorptionType.Passive:        if (!ShowPassiveAbsorption)  return; break;
                case AbsorptionType.StoppingVolume:  if (!ShowStoppingVolume)     return; break;
                case AbsorptionType.EffortVsResult:  if (!ShowEffortVsResult)     return; break;
            }
            // Part B: confluence gate — suppress raw markers below threshold
            var latestScore = _lastScorerResult;
            if (latestScore != null && latestScore.CategoryCount < MinArrowConfluence) return;
            // Part C: frequency limiter
            if (MaxSignalsPerSession > 0 && _sessionSignalCount >= MaxSignalsPerSession) return;

            // ── V8 visual: Clean diamond marker, NO text labels ──
            string tag = string.Format("ABS_{0}_{1}_{2}", barIdx, (int)s.Kind, s.Wick);
            int barsAgo = CurrentBar - barIdx;
            TrackRecentSignalDirection(s.Direction >= 0 ? 1 : -1);
            
            // Direction color: green for bull, red for bear (institutional palette)
            Brush brush = s.Direction >= 0 ? Brushes.LimeGreen : Brushes.Crimson;
            
            // Inner lane: ±2 ticks from wick
            double price = s.Direction >= 0 
                ? s.Price - 2 * TickSize   // below for bullish
                : s.Price + 2 * TickSize;  // above for bearish
            
            Draw.Diamond(this, tag, false, barsAgo, price, brush);
            _sessionSignalCount++;
        }
        ...
        private void DrawExhaustionMarker(int barIdx, ExhaustionSignal s)
        {
            ...
            switch (s.Kind)
            {
                case ExhaustionType.ZeroPrint:       if (!ShowZeroPrint)       return; break;
                case ExhaustionType.ExhaustionPrint:  if (!ShowExhaustionPrint) return; break;
                case ExhaustionType.ThinPrint:        if (!ShowThinPrint)       return; break;
                case ExhaustionType.FatPrint:         if (!ShowFatPrint)        return; break;
                case ExhaustionType.FadingMomentum:   if (!ShowFadingMomentum)  return; break;
                case ExhaustionType.BidAskFade:       if (!ShowBidAskFade)      return; break;
            }
            // Part B: strength gate
            if (s.Strength < MinExhaustionStrength) return;
            // Part B: confluence gate
            var latestScore = _lastScorerResult;
            if (latestScore != null && latestScore.CategoryCount < MinArrowConfluence) return;
            // Part C: frequency limiter
            if (MaxSignalsPerSession > 0 && _sessionSignalCount >= MaxSignalsPerSession) return;

            // ── V8 visual: Clean triangle marker, outer lane ──
            string tag = string.Format("EXH_{0}_{1}", barIdx, (int)s.Kind);
            int barsAgo = CurrentBar - barIdx;
            
            if (s.Direction > 0)
            {
                TrackRecentSignalDirection(1);
                // Outer lane: 4 ticks below wick, green
                Draw.TriangleUp(this, tag, false, barsAgo, s.Price - 4 * TickSize, Brushes.LimeGreen);
            }
            else if (s.Direction < 0)
            {
                TrackRecentSignalDirection(-1);
                // Outer lane: 4 ticks above wick, red
                Draw.TriangleDown(this, tag, false, barsAgo, s.Price + 4 * TickSize, Brushes.Crimson);
            }
            else
            {
                if (ShowBiasBox && !ShowRawPercentage)
                {
                    double bias = ComputeBiasScore(BiasLookback);
                    string biasText = bias > BiasLongThreshold ? "LONG" : bias < BiasShortThreshold ? "SHORT" : "—";
                    Brush biasColor = bias > BiasLongThreshold ? Brushes.LimeGreen
                        : bias < BiasShortThreshold ? Brushes.Crimson
                        : Brushes.DimGray;
                    string biasTag = "BiasBox_" + CurrentBar;
                    Draw.Text(this, biasTag, false, biasText, barsAgo, s.Price, 0,
                        Brushes.White, new SimpleFont("Arial", 9) { Bold = true },
                        System.Windows.TextAlignment.Center, biasColor, biasColor, 80);
                }
                else if (ShowRawPercentage)
                {
                    Draw.Diamond(this, tag, false, barsAgo, s.Price, Brushes.DimGray);
        ...
        [NinjaScriptProperty]
        [Display(Name = "Show Mission Control Panel", Order = 1, GroupName = "9. Mission Control")]
        public bool ShowMissionControl { get; set; }

        [NinjaScriptProperty]
        [Range(140, 360)]
        [Display(Name = "Panel Width (px)", Order = 2, GroupName = "9. Mission Control")]
        public int MissionControlWidth { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show ACTIVE SIGNAL Section", Order = 3, GroupName = "9. Mission Control")]
        public bool ShowMcActiveSignal { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Status Section", Order = 4, GroupName = "9. Mission Control")]
        public bool ShowMcStatus { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Day P&L Section", Order = 5, GroupName = "9. Mission Control")]
        public bool ShowMcDayPnL { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Position Section", Order = 6, GroupName = "9. Mission Control")]
        public bool ShowMcPosition { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Signals (44) Section", Order = 7, GroupName = "9. Mission Control")]
        public bool ShowMcSignalsList { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show FLATTEN ALL Action Bar", Order = 8, GroupName = "9. Mission Control")]
        public bool ShowMcActionBar { get; set; }

        // ▰▰▰ 3-tier signal clarity ▰▰▰
        [NinjaScriptProperty]
        [Display(Name = "TIER 1 Chart Overlay (setup/direction/trigger)", Order = 1, GroupName = "10. Signal Tiers")]
        public bool ShowTier1Overlay { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "TIER 3 Dots (informational noise — off by default)", Order = 2, GroupName = "10. Signal Tiers")]
        public bool ShowTier3Dots { get; set; }

        [NinjaScriptProperty]
        [Range(1, 50)]
        [Display(Name = "Active Signal Valid (bars)", Order = 3, GroupName = "10. Signal Tiers")]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Show Bias Box", Order = 15, GroupName = "2. Display")]
        public bool ShowBiasBox { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Raw Percentage", Order = 16, GroupName = "2. Display")]
        public bool ShowRawPercentage { get; set; }

        // ── Part A: granular per-variant visibility toggles ──

        [NinjaScriptProperty]
        [Display(Name = "Show Classic Absorption (ABS-01)", Order = 20, GroupName = "2. Display")]
        public bool ShowClassicAbsorption { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Passive Absorption (ABS-02)", Order = 21, GroupName = "2. Display")]
        public bool ShowPassiveAbsorption { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Stopping Volume (ABS-03)", Order = 22, GroupName = "2. Display")]
        public bool ShowStoppingVolume { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Effort vs Result (ABS-04)", Order = 23, GroupName = "2. Display")]
        public bool ShowEffortVsResult { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Zero Print (EXH-01)", Order = 24, GroupName = "2. Display")]
        public bool ShowZeroPrint { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Exhaustion Print (EXH-02)", Order = 25, GroupName = "2. Display")]
        public bool ShowExhaustionPrint { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Thin Print (EXH-03)", Order = 26, GroupName = "2. Display")]
        public bool ShowThinPrint { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Fat Print (EXH-04)", Order = 27, GroupName = "2. Display")]
        public bool ShowFatPrint { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Fading Momentum (EXH-05)", Order = 28, GroupName = "2. Display")]
        public bool ShowFadingMomentum { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Bid/Ask Fade (EXH-06)", Order = 29, GroupName = "2. Display")]
        public bool ShowBidAskFade { get; set; }

        // ── Part B: arrow gating display toggles ──

        [NinjaScriptProperty]
        [Display(Name = "Show Triggered Arrows", Order = 30, GroupName = "2. Display")]
        public bool ShowTriggeredArrows { get; set; }

        [NinjaScriptProperty]
        [Range(7f, 16f)]
        [Display(Name = "Cell Font Size", Order = 40, GroupName = "2. Display")]
        public float CellFontSize { get; set; }

        [NinjaScriptProperty]
        [Range(40, 200)]
        [Display(Name = "Cell Column Width (px)", Order = 41, GroupName = "2. Display")]
        public int CellColumnWidth { get; set; }

        // ── Part B: arrow gating thresholds (3. Signals group) ──

        [NinjaScriptProperty]
        [Range(1, 7)]
        [Display(Name = "Min Arrow Confluence", Order = 1, GroupName = "3. Signals",
                 Description = "Minimum CategoryCount from scorer before raw absorption/exhaustion markers render")]
        public int MinArrowConfluence { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 1.0)]
        [Display(Name = "Min Exhaustion Strength", Order = 2, GroupName = "3. Signals",
                 Description = "Minimum strength (0-1) for exhaustion markers to render")]
        public double MinExhaustionStrength { get; set; }

        [NinjaScriptProperty]
        [Range(0.0, 1.0)]
        [Display(Name = "Bias Long Threshold", Order = 3, GroupName = "3. Signals",
                 Description = "Bias score above this threshold renders the LONG box")]
        public double BiasLongThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(-1.0, 0.0)]
        [Display(Name = "Bias Short Threshold", Order = 4, GroupName = "3. Signals",
                 Description = "Bias score below this threshold renders the SHORT box")]
        public double BiasShortThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(1, 10)]
        [Display(Name = "Bias Lookback", Order = 5, GroupName = "3. Signals",
                 Description = "Recent directional signals used to normalize the bias score")]
        public int BiasLookback { get; set; }

        // ── Part C: signal frequency limiter ──

        [NinjaScriptProperty]
        [Range(0, 50)]
        [Display(Name = "Max Signals Per Session", Order = 6, GroupName = "3. Signals",
                 Description = "Maximum draw-markers per RTH session (0 = unlimited). Resets on IsFirstBarOfSession.")]
        public int MaxSignalsPerSession { get; set; }

        // --- Profile Anchor Levels ---

        [NinjaScriptProperty]
        [Display(Name = "Show Profile Anchors", Order = 20, GroupName = "4. Profile Anchors")]
        public bool ShowProfileAnchors { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Prior-Day Levels", Order = 21, GroupName = "4. Profile Anchors")]
        public bool ShowPriorDayLevels { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Naked POCs", Order = 22, GroupName = "4. Profile Anchors")]
        public bool ShowNakedPocs { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Composite 5-Day VA", Order = 23, GroupName = "4. Profile Anchors")]
        public bool ShowCompositeVA { get; set; }

        [NinjaScriptProperty]
        [Range(1, 60)]
        [Display(Name = "Naked POC Max Age (sessions)", Order = 24, GroupName = "4. Profile Anchors")]
        public int NakedPocMaxAgeSessions { get; set; }

        // Anchor brush properties

        [XmlIgnore]
        [Display(Name = "Anchor POC Color",       Order = 40, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Anchor VA Color",        Order = 41, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Anchor Naked POC Color", Order = 42, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Anchor PW POC Color",    Order = 43, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Anchor Composite VA",    Order = 44, GroupName = "5. Colors")]
        ...
        [NinjaScriptProperty]
        [Display(Name = "Show Liquidity Walls (Rithmic L2)", Order = 30, GroupName = "6. Liquidity (L2)")]
        public bool ShowLiquidityWalls { get; set; }

        [NinjaScriptProperty]
        [Range(10, 5000)]
        [Display(Name = "Wall Min Size (contracts)", Order = 31, GroupName = "6. Liquidity (L2)")]
        public int LiquidityWallMin { get; set; }

        [NinjaScriptProperty]
        [Range(10, 600)]
        [Display(Name = "Wall Stale (seconds)", Order = 32, GroupName = "6. Liquidity (L2)",
        ...
        [NinjaScriptProperty]
        [Range(1, 10)]
        [Display(Name = "Max Walls Per Side", Order = 33, GroupName = "6. Liquidity (L2)")]
        public int LiquidityMaxPerSide { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Chart Trader Toolbar", Order = 40, GroupName = "7. Chart Trader",
        ...
        [NinjaScriptProperty]
        [Display(Name = "Show Score HUD", Order = 1, GroupName = "8. DEEP6 Scorer",
        ...
        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Score HUD Padding (px)", Order = 2, GroupName = "8. DEEP6 Scorer",
        ...
        [XmlIgnore]
        [Display(Name = "Bid Cell Color",      Order = 30, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Ask Cell Color",      Order = 31, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Cell Text Color",     Order = 32, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "POC Color",           Order = 33, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "VAH Color",           Order = 34, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "VAL Color",           Order = 35, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Imbalance Buy Color", Order = 36, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Imbalance Sell Color",Order = 37, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Wall Bid (resting buy)",  Order = 50, GroupName = "5. Colors")]
        ...
        [XmlIgnore]
        [Display(Name = "Wall Ask (resting sell)", Order = 51, GroupName = "5. Colors")]
