...
{
    public class DEEP6CompoundSignalsV2 : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "DEEP6CompoundSignalsV2";
                Description = "DEEP6 compound signal indicator V2: reduced-frequency, higher-conviction reversal selection for NQ.";
        ...
        private SignalDecision BuildBullishDecision(VolumetricSnapshot snapshot)
        {
            ...
            int anchorCount = 0;
            anchorCount += absorptionProxy ? 1 : 0;
            anchorCount += bullishCvdDiv ? 1 : 0;
            anchorCount += bullishFailedOR ? 1 : 0;
            anchorCount += (bullishNarrowing && bullishCvdDiv) ? 1 : 0;
            anchorCount += (bullishDoji && adaptiveLowDelta) ? 1 : 0;
            anchorCount += (stableVol && firstHour && notKillers) ? 1 : 0;

            bool premiumCombo = (bullishCvdDiv && bullishDoji && firstHour && notKillers)
                || (bullishNarrowing && bullishCvdDiv && firstHour && notKillers)
                || (absorptionProxy && firstHour && notLunch)
                || (bullishFailedOR && bullishCvdDiv && firstHour)
                || (adaptiveLowDelta && bullishDoji && firstHour && notKillers);

            if (!firstHour || !notKillers)
                return d;
            if (anchorCount < 2)
                return d;
        ...
        private SignalDecision BuildBearishDecision(VolumetricSnapshot snapshot)
        {
            ...
            int anchorCount = 0;
            anchorCount += absorptionProxy ? 1 : 0;
            anchorCount += bearishCvdDiv ? 1 : 0;
            anchorCount += bearishFailedOR ? 1 : 0;
            anchorCount += (bearishNarrowing && bearishCvdDiv) ? 1 : 0;
            anchorCount += (bearishDoji && adaptiveLowDelta) ? 1 : 0;
            anchorCount += (stableVol && firstHour && notKillers) ? 1 : 0;

            bool premiumCombo = (bearishCvdDiv && bearishDoji && firstHour && notKillers)
                || (bearishNarrowing && bearishCvdDiv && firstHour && notKillers)
                || (absorptionProxy && firstHour && notLunch)
                || (bearishFailedOR && bearishCvdDiv && firstHour)
                || (adaptiveLowDelta && bearishDoji && firstHour && notKillers);
        ...
        {
            return "DEEP6CompoundSignalsV2_" + side + "_T" + tier.ToString(CultureInfo.InvariantCulture) + "_" + CurrentBar.ToString(CultureInfo.InvariantCulture);
        }
