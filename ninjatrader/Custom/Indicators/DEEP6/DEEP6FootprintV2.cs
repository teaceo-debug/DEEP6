...
//   %USERPROFILE%\Documents\NinjaTrader 8\bin\Custom\Indicators\DEEP6\DEEP6FootprintV2.cs
...
namespace NinjaTrader.NinjaScript.Indicators.DEEP6
{
    public class DEEP6FootprintV2 : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description         = "DEEP6 Footprint V2 — separate Version Two footprint path with setup, direction, and trigger visuals only.";
                Name                = "DEEP6 Footprint V2";
            ...
            for (int barIdx = fromIdx; barIdx <= toIdx; barIdx++)
            {
                ...
                if (ShowFootprintCells)
                {
                    ...
                    foreach (var kv in fbar.Levels)
                    {
                        ...
                        // V2: Heatmap mode — cell intensity scales with volume (renders before imbalance fill)
                ...
                // V2: Stacked zone boxes (rendered on top of cells, below POC/VA)
                if (ShowStackedZoneBoxes)
                {
                    ...
                    if (zones != null)
                    {
                        ...
                        {
                            float yTop    = chartScale.GetYByValue(zone.PriceHigh + tickSize * 0.5);
                            float yBottom = chartScale.GetYByValue(zone.PriceLow  - tickSize * 0.5);
                            float sw = zone.Tier == 3 ? 2f : zone.Tier == 2 ? 1.5f : 1f;
                            var zBrush = zone.Direction > 0 ? _stackedBuyZoneDx : _stackedSellZoneDx;
                            if (zBrush != null)
                                RenderTarget.DrawRectangle(
                                    new RectangleF(xLeft, yTop, colW, yBottom - yTop), zBrush, sw);
                        }
                    }
                }

                // V2: Volume Climax bar marker — bold horizontal line at extreme
                if (ShowVolumeClimax)
                {
                    int climDir;
                    ...
                    {
                        float yExtreme = chartScale.GetYByValue(climDir > 0 ? fbar.Low : fbar.High);
                        float offset = climDir > 0 ? 2f : -2f;
                        RenderTarget.DrawLine(
                            new Vector2(xLeft, yExtreme + offset),
                            new Vector2(xLeft + colW, yExtreme + offset),
                            _volumeClimaxDx, 2.5f);
                    }
                }
        ...
        private void RenderTier1Overlay(ChartControl cc, ChartScale cs, float panelLeftEdge)
        {
            ...
            string priceText = string.Format("{0:F2}", sr.EntryPrice);
            DrawHaloText(stateText, _pwPillValueFont, accent, xRight - 160f, yEntry - 18f, 150f, 16f);
            DrawHaloText(priceText, _pwPillLabelFont, _pwAeroWhiteDx, xRight - 80f, yEntry - 4f, 70f, 12f);
        ...
        private void DrawScorerTierMarker(int barIdx, ScorerResult scored)
        {
            ...
            // Version Two: do not auto-draw an entry arrow on the next candle.
            // The gray/shape marker is the setup; Armed setups get an explicit WAIT LONG/WAIT SHORT
            // annotation. Triggered BUY/SELL arrows will be added only after a real confirmation
            // state machine is wired in a later step.
            ...
            switch (scored.Tier)
            {
                case SignalTier.TYPE_A:
                {
                    ...
                    // Version Two: narrative/armed label — pushed further out so it doesn't collide with score text.
                    double lblPrice = isLong ? markerPrice - 8.0 * TickSize : markerPrice + 8.0 * TickSize;
                    string narrative = armedLabel.Length > 0 ? armedLabel : TruncateEllipsis(scored.Narrative ?? string.Empty, 50);
                    if (narrative.Length > 0)
                ...
                case SignalTier.TYPE_B:
                {
                    ...
                    // Version Two: TypeB also stays in setup/armed mode only. No auto-entry arrow yet.
                    if (armedLabel.Length > 0)
                    {
                        double lblPrice = isLong ? markerPrice - 6.0 * TickSize : markerPrice + 6.0 * TickSize;
                        Draw.Text(this, "SCORE_LBL_" + suffix, armedLabel, barsAgo, lblPrice, pick);
                    }
                    break;
                }
...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    {
        private DEEP6.DEEP6FootprintV2[] cacheDEEP6FootprintV2;
        public DEEP6.DEEP6FootprintV2 DEEP6FootprintV2(double imbalanceRatio, double absorbWickMinPct, double exhaustWickMinPct, bool showFootprintCells, bool showPoc, bool showValueArea, bool showAbsorptionMarkers, bool showExhaustionMarkers, float cellFontSize, int cellColumnWidth, bool showProfileAnchors, bool showPriorDayLevels, bool showNakedPocs, bool showCompositeVA, int nakedPocMaxAgeSessions, bool showLiquidityWalls, int liquidityWallMin, int liquidityWallStaleSec, int liquidityMaxPerSide, bool showChartTrader)
        {
            return DEEP6FootprintV2(Input, imbalanceRatio, absorbWickMinPct, exhaustWickMinPct, showFootprintCells, showPoc, showValueArea, showAbsorptionMarkers, showExhaustionMarkers, cellFontSize, cellColumnWidth, showProfileAnchors, showPriorDayLevels, showNakedPocs, showCompositeVA, nakedPocMaxAgeSessions, showLiquidityWalls, liquidityWallMin, liquidityWallStaleSec, liquidityMaxPerSide, showChartTrader);
        }

        public DEEP6.DEEP6FootprintV2 DEEP6FootprintV2(ISeries<double> input, double imbalanceRatio, double absorbWickMinPct, double exhaustWickMinPct, bool showFootprintCells, bool showPoc, bool showValueArea, bool showAbsorptionMarkers, bool showExhaustionMarkers, float cellFontSize, int cellColumnWidth, bool showProfileAnchors, bool showPriorDayLevels, bool showNakedPocs, bool showCompositeVA, int nakedPocMaxAgeSessions, bool showLiquidityWalls, int liquidityWallMin, int liquidityWallStaleSec, int liquidityMaxPerSide, bool showChartTrader)
        {
            if (cacheDEEP6FootprintV2 != null)
                for (int idx = 0; idx < cacheDEEP6FootprintV2.Length; idx++)
                    if (cacheDEEP6FootprintV2[idx] != null && cacheDEEP6FootprintV2[idx].ImbalanceRatio == imbalanceRatio && cacheDEEP6FootprintV2[idx].AbsorbWickMinPct == absorbWickMinPct && cacheDEEP6FootprintV2[idx].ExhaustWickMinPct == exhaustWickMinPct && cacheDEEP6FootprintV2[idx].ShowFootprintCells == showFootprintCells && cacheDEEP6FootprintV2[idx].ShowPoc == showPoc && cacheDEEP6FootprintV2[idx].ShowValueArea == showValueArea && cacheDEEP6FootprintV2[idx].ShowAbsorptionMarkers == showAbsorptionMarkers && cacheDEEP6FootprintV2[idx].ShowExhaustionMarkers == showExhaustionMarkers && cacheDEEP6FootprintV2[idx].CellFontSize == cellFontSize && cacheDEEP6FootprintV2[idx].CellColumnWidth == cellColumnWidth && cacheDEEP6FootprintV2[idx].ShowProfileAnchors == showProfileAnchors && cacheDEEP6FootprintV2[idx].ShowPriorDayLevels == showPriorDayLevels && cacheDEEP6FootprintV2[idx].ShowNakedPocs == showNakedPocs && cacheDEEP6FootprintV2[idx].ShowCompositeVA == showCompositeVA && cacheDEEP6FootprintV2[idx].NakedPocMaxAgeSessions == nakedPocMaxAgeSessions && cacheDEEP6FootprintV2[idx].ShowLiquidityWalls == showLiquidityWalls && cacheDEEP6FootprintV2[idx].LiquidityWallMin == liquidityWallMin && cacheDEEP6FootprintV2[idx].LiquidityWallStaleSec == liquidityWallStaleSec && cacheDEEP6FootprintV2[idx].LiquidityMaxPerSide == liquidityMaxPerSide && cacheDEEP6FootprintV2[idx].ShowChartTrader == showChartTrader && cacheDEEP6FootprintV2[idx].EqualsInput(input))
                        return cacheDEEP6FootprintV2[idx];
            return CacheIndicator<DEEP6.DEEP6FootprintV2>(new DEEP6.DEEP6FootprintV2() { ImbalanceRatio = imbalanceRatio, AbsorbWickMinPct = absorbWickMinPct, ExhaustWickMinPct = exhaustWickMinPct, ShowFootprintCells = showFootprintCells, ShowPoc = showPoc, ShowValueArea = showValueArea, ShowAbsorptionMarkers = showAbsorptionMarkers, ShowExhaustionMarkers = showExhaustionMarkers, CellFontSize = cellFontSize, CellColumnWidth = cellColumnWidth, ShowProfileAnchors = showProfileAnchors, ShowPriorDayLevels = showPriorDayLevels, ShowNakedPocs = showNakedPocs, ShowCompositeVA = showCompositeVA, NakedPocMaxAgeSessions = nakedPocMaxAgeSessions, ShowLiquidityWalls = showLiquidityWalls, LiquidityWallMin = liquidityWallMin, LiquidityWallStaleSec = liquidityWallStaleSec, LiquidityMaxPerSide = liquidityMaxPerSide, ShowChartTrader = showChartTrader }, input, ref cacheDEEP6FootprintV2);
        }
    }
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
    ...
    {
        public Indicators.DEEP6.DEEP6FootprintV2 DEEP6FootprintV2(double imbalanceRatio, double absorbWickMinPct, double exhaustWickMinPct, bool showFootprintCells, bool showPoc, bool showValueArea, bool showAbsorptionMarkers, bool showExhaustionMarkers, float cellFontSize, int cellColumnWidth, bool showProfileAnchors, bool showPriorDayLevels, bool showNakedPocs, bool showCompositeVA, int nakedPocMaxAgeSessions, bool showLiquidityWalls, int liquidityWallMin, int liquidityWallStaleSec, int liquidityMaxPerSide, bool showChartTrader)
        {
            return indicator.DEEP6FootprintV2(Input, imbalanceRatio, absorbWickMinPct, exhaustWickMinPct, showFootprintCells, showPoc, showValueArea, showAbsorptionMarkers, showExhaustionMarkers, cellFontSize, cellColumnWidth, showProfileAnchors, showPriorDayLevels, showNakedPocs, showCompositeVA, nakedPocMaxAgeSessions, showLiquidityWalls, liquidityWallMin, liquidityWallStaleSec, liquidityMaxPerSide, showChartTrader);
        }
