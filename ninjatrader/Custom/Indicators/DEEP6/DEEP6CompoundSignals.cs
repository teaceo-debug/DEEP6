...
{
    public class DEEP6CompoundSignals : Indicator
    {
        private const string LogPrefix = "[DEEP6CompoundSignals]";
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "DEEP6CompoundSignals";
                Description = "DEEP6 compound signal indicator built from validated 60m extreme + 15m trend signal families for NQ.";
                ...
                LowDeltaVolThreshold = 0.05;
                DeltaVolQuantileLookback = 50;
                StableVolLookback = 40;
                VolOfVolLookback = 10;
                StableVolPercentile = 0.25;
        ...
        private SignalDecision BuildBullishDecision(VolumetricSnapshot snapshot)
        {
            ...
            bool bullishNarrowing = narrowing && snapshot.BarDelta > 0;
            bool bullishHammer = IsBullishHammer();
            bool bullishEngulfing = IsBullishEngulfing();
            bool bullishMorningStar = IsBullishMorningStar();
            bool bullishFailedOR = IsBullishFailedOpeningRangeBreakdown();
            bool stableVol = IsStableVolatility();
            bool lowDeltaVol = IsLowDeltaVolume(snapshot);
            bool adaptiveLowDelta = IsAdaptiveLowDeltaVolume(snapshot);
            bool smallOvernight = IsSmallOvernightMove();
            bool absorptionProxy = IsBullishAbsorptionProxy(snapshot);
            bool priorWideRangeDay = IsPriorWideRangeDay();
            bool weeklyBreakout = IsWeeklyBreakout();
            bool priorLosingSession = _sessionOpen < _priorSessionClose;
            bool notMonday = Time[0].DayOfWeek != DayOfWeek.Monday;
            bool notSummer = Time[0].Month < 6 || Time[0].Month > 8;
            bool notFomc = !IsApproxFomcWindow();
            bool firstHour = IsFirstHour();
            bool notLunch = !IsLunch();

            int score = 45;
            score += firstHour ? 10 : 0;
            score += notKillers ? 10 : -8;
            score += stableVol ? 8 : 0;
            score += bullishDoji ? 8 : 0;
            score += bullishCvdDiv ? 12 : 0;
            score += bullishNarrowing ? 7 : 0;
            score += bullishHammer ? 6 : 0;
            score += bullishEngulfing ? 6 : 0;
            score += bullishMorningStar ? 7 : 0;
            score += bullishFailedOR ? 6 : 0;
            score += lowDeltaVol ? 7 : 0;
            score += adaptiveLowDelta ? 8 : 0;
            score += smallOvernight ? 5 : 0;
            score += absorptionProxy ? 12 : 0;
            score += priorWideRangeDay ? 5 : 0;
            score += weeklyBreakout ? 5 : 0;
            score += priorLosingSession ? 4 : 0;
            score += notMonday ? 3 : 0;
            score += notSummer ? 2 : 0;
            score += notFomc ? 2 : 0;
            score += notLunch ? 2 : -4;

            bool premiumCombo = (stableVol && firstHour && notKillers)
                || (bullishCvdDiv && bullishDoji && firstHour && notKillers)
                || (bullishNarrowing && bullishCvdDiv && notKillers)
                || (absorptionProxy && notLunch)
                || (adaptiveLowDelta && bullishDoji && notKillers);

            if (premiumCombo)
                score += 8;

            int tier = ResolveTier(score, premiumCombo, absorptionProxy);
            ...
            d.Price = Low[0] - ((tier == 1 ? 4 : tier == 2 ? 6 : 8) * TickSize);
            d.Label = BuildBullishLabel(stableVol, bullishDoji, bullishCvdDiv, bullishNarrowing, absorptionProxy, bullishFailedOR, adaptiveLowDelta);
            return d;
        }
        ...
        private SignalDecision BuildBearishDecision(VolumetricSnapshot snapshot)
        {
            ...
            bool narrowing = IsThreeNarrowingRanges();
            bool bearishNarrowing = narrowing && snapshot.BarDelta < 0;
            bool bearishShootingStar = IsBearishShootingStar();
            bool bearishEngulfing = IsBearishEngulfing();
            bool bearishEveningStar = IsBearishEveningStar();
            bool bearishFailedOR = IsBearishFailedOpeningRangeBreakout();
            bool stableVol = IsStableVolatility();
            bool lowDeltaVol = IsLowDeltaVolume(snapshot);
            bool adaptiveLowDelta = IsAdaptiveLowDeltaVolume(snapshot);
            bool smallOvernight = IsSmallOvernightMove();
            bool absorptionProxy = IsBearishAbsorptionProxy(snapshot);
            bool priorWideRangeDay = IsPriorWideRangeDay();
            bool weeklyBreakout = IsWeeklyBreakout();
            bool priorLosingSession = _sessionOpen > _priorSessionClose;
            bool notMonday = Time[0].DayOfWeek != DayOfWeek.Monday;
            ...
            score += notKillers ? 10 : -8;
            score += stableVol ? 8 : 0;
            score += bearishDoji ? 8 : 0;
            score += bearishCvdDiv ? 12 : 0;
            score += bearishNarrowing ? 7 : 0;
            score += bearishShootingStar ? 6 : 0;
            score += bearishEngulfing ? 6 : 0;
            score += bearishEveningStar ? 7 : 0;
            score += bearishFailedOR ? 6 : 0;
            score += lowDeltaVol ? 7 : 0;
            score += adaptiveLowDelta ? 8 : 0;
            score += smallOvernight ? 5 : 0;
            score += absorptionProxy ? 12 : 0;
            ...
            bool premiumCombo = (stableVol && firstHour && notKillers)
                || (bearishCvdDiv && bearishDoji && firstHour && notKillers)
                || (bearishNarrowing && bearishCvdDiv && notKillers)
                || (absorptionProxy && notLunch)
                || (adaptiveLowDelta && bearishDoji && notKillers);
            ...
            d.Price = High[0] + ((tier == 1 ? 4 : tier == 2 ? 6 : 8) * TickSize);
            d.Label = BuildBearishLabel(stableVol, bearishDoji, bearishCvdDiv, bearishNarrowing, absorptionProxy, bearishFailedOR, adaptiveLowDelta);
            return d;
        }
        ...
        {
            return "DEEP6CompoundSignals_" + side + "_T" + tier.ToString(CultureInfo.InvariantCulture) + "_" + CurrentBar.ToString(CultureInfo.InvariantCulture);
        }
        ...
        [NinjaScriptProperty]
        [Range(0.05, 0.40)]
        [Display(Name = "ExtremeThreshold", GroupName = "Logic", Order = 8)]
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private DEEP6.DEEP6CompoundSignals[] cacheDEEP6CompoundSignals;
		public DEEP6.DEEP6CompoundSignals DEEP6CompoundSignals(bool enableTier1, bool enableTier2, bool enableTier3, bool enableAlerts, bool showPatternLabels, bool excludeLunch, bool firstHourOnly, bool showKillerZone, double extremeThreshold, double middleRangeLow, double middleRangeHigh, double volumeSpikeMultiplier, double lowDeltaVolThreshold, int deltaVolQuantileLookback, int stableVolLookback, int volOfVolLookback, double stableVolPercentile, double smallOvernightMovePoints, int scoreTier1, int scoreTier2, int scoreTier3, int stopTicks, int absorptionStopTicks)
		{
			return DEEP6CompoundSignals(Input, enableTier1, enableTier2, enableTier3, enableAlerts, showPatternLabels, excludeLunch, firstHourOnly, showKillerZone, extremeThreshold, middleRangeLow, middleRangeHigh, volumeSpikeMultiplier, lowDeltaVolThreshold, deltaVolQuantileLookback, stableVolLookback, volOfVolLookback, stableVolPercentile, smallOvernightMovePoints, scoreTier1, scoreTier2, scoreTier3, stopTicks, absorptionStopTicks);
		}

		public DEEP6.DEEP6CompoundSignals DEEP6CompoundSignals(ISeries<double> input, bool enableTier1, bool enableTier2, bool enableTier3, bool enableAlerts, bool showPatternLabels, bool excludeLunch, bool firstHourOnly, bool showKillerZone, double extremeThreshold, double middleRangeLow, double middleRangeHigh, double volumeSpikeMultiplier, double lowDeltaVolThreshold, int deltaVolQuantileLookback, int stableVolLookback, int volOfVolLookback, double stableVolPercentile, double smallOvernightMovePoints, int scoreTier1, int scoreTier2, int scoreTier3, int stopTicks, int absorptionStopTicks)
		{
			if (cacheDEEP6CompoundSignals != null)
				for (int idx = 0; idx < cacheDEEP6CompoundSignals.Length; idx++)
					if (cacheDEEP6CompoundSignals[idx] != null && cacheDEEP6CompoundSignals[idx].EnableTier1 == enableTier1 && cacheDEEP6CompoundSignals[idx].EnableTier2 == enableTier2 && cacheDEEP6CompoundSignals[idx].EnableTier3 == enableTier3 && cacheDEEP6CompoundSignals[idx].EnableAlerts == enableAlerts && cacheDEEP6CompoundSignals[idx].ShowPatternLabels == showPatternLabels && cacheDEEP6CompoundSignals[idx].ExcludeLunch == excludeLunch && cacheDEEP6CompoundSignals[idx].FirstHourOnly == firstHourOnly && cacheDEEP6CompoundSignals[idx].ShowKillerZone == showKillerZone && cacheDEEP6CompoundSignals[idx].ExtremeThreshold == extremeThreshold && cacheDEEP6CompoundSignals[idx].MiddleRangeLow == middleRangeLow && cacheDEEP6CompoundSignals[idx].MiddleRangeHigh == middleRangeHigh && cacheDEEP6CompoundSignals[idx].VolumeSpikeMultiplier == volumeSpikeMultiplier && cacheDEEP6CompoundSignals[idx].LowDeltaVolThreshold == lowDeltaVolThreshold && cacheDEEP6CompoundSignals[idx].DeltaVolQuantileLookback == deltaVolQuantileLookback && cacheDEEP6CompoundSignals[idx].StableVolLookback == stableVolLookback && cacheDEEP6CompoundSignals[idx].VolOfVolLookback == volOfVolLookback && cacheDEEP6CompoundSignals[idx].StableVolPercentile == stableVolPercentile && cacheDEEP6CompoundSignals[idx].SmallOvernightMovePoints == smallOvernightMovePoints && cacheDEEP6CompoundSignals[idx].ScoreTier1 == scoreTier1 && cacheDEEP6CompoundSignals[idx].ScoreTier2 == scoreTier2 && cacheDEEP6CompoundSignals[idx].ScoreTier3 == scoreTier3 && cacheDEEP6CompoundSignals[idx].StopTicks == stopTicks && cacheDEEP6CompoundSignals[idx].AbsorptionStopTicks == absorptionStopTicks && cacheDEEP6CompoundSignals[idx].EqualsInput(input))
						return cacheDEEP6CompoundSignals[idx];
			return CacheIndicator<DEEP6.DEEP6CompoundSignals>(new DEEP6.DEEP6CompoundSignals(){ EnableTier1 = enableTier1, EnableTier2 = enableTier2, EnableTier3 = enableTier3, EnableAlerts = enableAlerts, ShowPatternLabels = showPatternLabels, ExcludeLunch = excludeLunch, FirstHourOnly = firstHourOnly, ShowKillerZone = showKillerZone, ExtremeThreshold = extremeThreshold, MiddleRangeLow = middleRangeLow, MiddleRangeHigh = middleRangeHigh, VolumeSpikeMultiplier = volumeSpikeMultiplier, LowDeltaVolThreshold = lowDeltaVolThreshold, DeltaVolQuantileLookback = deltaVolQuantileLookback, StableVolLookback = stableVolLookback, VolOfVolLookback = volOfVolLookback, StableVolPercentile = stableVolPercentile, SmallOvernightMovePoints = smallOvernightMovePoints, ScoreTier1 = scoreTier1, ScoreTier2 = scoreTier2, ScoreTier3 = scoreTier3, StopTicks = stopTicks, AbsorptionStopTicks = absorptionStopTicks }, input, ref cacheDEEP6CompoundSignals);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.DEEP6.DEEP6CompoundSignals DEEP6CompoundSignals(bool enableTier1, bool enableTier2, bool enableTier3, bool enableAlerts, bool showPatternLabels, bool excludeLunch, bool firstHourOnly, bool showKillerZone, double extremeThreshold, double middleRangeLow, double middleRangeHigh, double volumeSpikeMultiplier, double lowDeltaVolThreshold, int deltaVolQuantileLookback, int stableVolLookback, int volOfVolLookback, double stableVolPercentile, double smallOvernightMovePoints, int scoreTier1, int scoreTier2, int scoreTier3, int stopTicks, int absorptionStopTicks)
		{
			return indicator.DEEP6CompoundSignals(Input, enableTier1, enableTier2, enableTier3, enableAlerts, showPatternLabels, excludeLunch, firstHourOnly, showKillerZone, extremeThreshold, middleRangeLow, middleRangeHigh, volumeSpikeMultiplier, lowDeltaVolThreshold, deltaVolQuantileLookback, stableVolLookback, volOfVolLookback, stableVolPercentile, smallOvernightMovePoints, scoreTier1, scoreTier2, scoreTier3, stopTicks, absorptionStopTicks);
		}

		public Indicators.DEEP6.DEEP6CompoundSignals DEEP6CompoundSignals(ISeries<double> input , bool enableTier1, bool enableTier2, bool enableTier3, bool enableAlerts, bool showPatternLabels, bool excludeLunch, bool firstHourOnly, bool showKillerZone, double extremeThreshold, double middleRangeLow, double middleRangeHigh, double volumeSpikeMultiplier, double lowDeltaVolThreshold, int deltaVolQuantileLookback, int stableVolLookback, int volOfVolLookback, double stableVolPercentile, double smallOvernightMovePoints, int scoreTier1, int scoreTier2, int scoreTier3, int stopTicks, int absorptionStopTicks)
		{
			return indicator.DEEP6CompoundSignals(input, enableTier1, enableTier2, enableTier3, enableAlerts, showPatternLabels, excludeLunch, firstHourOnly, showKillerZone, extremeThreshold, middleRangeLow, middleRangeHigh, volumeSpikeMultiplier, lowDeltaVolThreshold, deltaVolQuantileLookback, stableVolLookback, volOfVolLookback, stableVolPercentile, smallOvernightMovePoints, scoreTier1, scoreTier2, scoreTier3, stopTicks, absorptionStopTicks);
		}
	}
