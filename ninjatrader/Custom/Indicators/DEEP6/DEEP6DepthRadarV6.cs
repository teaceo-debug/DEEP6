// DEEP6 DepthRadar V6 — schema-v2 MBO evidence/ranking JSON renderer.
//
// V6 extends V4 with:
...
{
    public class DEEP6DepthRadarV6 : Indicator
    {
        ...
        private TextFormat _decisionFont;
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                MaxDistanceFromPriceTicks = 120;
                DecisionQualityThreshold = 60;
                SpoofRiskThreshold = 60;
                IcebergThreshold = 50;
                BreakRiskThreshold = 70;
            ...
            else if (State == State.DataLoaded)
            {
                ...
                Print("[DEEP6DepthRadarV6] DataLoaded — JSON path: " + WallsJsonPath + " exists=" + File.Exists(WallsJsonPath));
            }
            ...
            {
                Print("[DEEP6DepthRadarV6] Realtime — reading Rithmic DOM data from JSON: " + WallsJsonPath);
            }
        ...
        private void OnJsonCheck(object state)
        {
            ...
            {
                Print("[DEEP6DepthRadarV6] JSON read failed: " + ex.Message);
            }
        }
        ...
        private bool ReadWallsFromJson()
        {
            ...
            if (updated)
            {
                ...
                Print(string.Format("[DEEP6DepthRadarV6] JSON loaded {0} bids + {1} asks (WallMinSize={2})",
        ...
        public override void OnRenderTargetChanged()
        {
            ...
            _labelFont = new TextFormat(NinjaTrader.Core.Globals.DirectWriteFactory, "Segoe UI", Math.Max(10, LabelFontSize))
            {
                TextAlignment = SharpDX.DirectWrite.TextAlignment.Trailing,
                ParagraphAlignment = ParagraphAlignment.Center,
            };
            _hudFont = new TextFormat(NinjaTrader.Core.Globals.DirectWriteFactory, "Consolas", Math.Max(10, HudFontSize));
            _decisionFont = new TextFormat(NinjaTrader.Core.Globals.DirectWriteFactory, "Consolas", Math.Max(10, DecisionFontSize));
            _markerFont = new TextFormat(NinjaTrader.Core.Globals.DirectWriteFactory, "Segoe UI Symbol", Math.Max(10, LabelFontSize))
            {
                TextAlignment = SharpDX.DirectWrite.TextAlignment.Trailing,
                ParagraphAlignment = ParagraphAlignment.Center,
            };
        ...
        private struct RenderSummary
        {
            ...
            public float BestBidQuality;
            public float BestAskQuality;
            public float BestBidIceberg;
            public float BestAskIceberg;
            public float BestBidBreakRisk;
            public float BestAskBreakRisk;
            public double BestBidDistanceTicks;
            public double BestAskDistanceTicks;
        ...
        private void RenderWalls(ChartScale cs, float panelRight)
        {
            ...
            double referencePrice = _payloadMidPrice > 0 ? _payloadMidPrice : (CurrentBar >= 0 ? Close[0] : 0.0);
            ...
            if (ShowBids)
                summary.BidCount = DrawWallsForSide(cs, bidSnap, true, fresh, minVis, maxVis, panelRight, referencePrice, ref summary);
            if (ShowAsks)
                summary.AskCount = DrawWallsForSide(cs, askSnap, false, fresh, minVis, maxVis, panelRight, referencePrice, ref summary);

            Interlocked.Exchange(ref _animateWalls, summary.HasAnimation ? 1 : 0);
            DrawHud(summary);
            DrawDecisionPanel(summary, referencePrice);
        }
            ...
            ref RenderSummary summary)
        {
            ...
            foreach (var kv in snap)
            {
                ...
                if (referencePrice > 0 && MaxDistanceFromPriceTicks > 0)
                {
                    double distTicks = Math.Abs(kv.Key - referencePrice) / Math.Max(TickSize, 0.0000001);
                    if (distTicks > MaxDistanceFromPriceTicks) continue;
                }
                walls.Add(kv);
            }
            ...
            float labelFontSize = Math.Max(10f, LabelFontSize);
            float labelHeight = labelFontSize + 10f;
            float markerWidth = Math.Max(26f, labelFontSize + 14f);
            float labelWidth = Math.Max(240f, LabelPanelWidth);
            float labelMinGap = Math.Max(18f, labelHeight + 2f);
            ...
            for (int i = 0; i < show; i++)
            {
                ...
                if (st.MaxSize >= GlowThreshold && glowBrushes != null)
                {
                    for (int g = 0; g < glowBrushes.Length; g++)
                        DrawLineWithOpacity(glowBrushes[g], alpha, (float)ChartPanel.X, y, panelRight - labelWidth - markerWidth - 12f, y, GLOW_WIDTHS[g]);
                }

                DrawLineWithOpacity(brush, alpha, (float)ChartPanel.X, y, panelRight - labelWidth - markerWidth - 12f, y, thickness);

                if (ShowLabels)
                {
                    float labelY = y - (labelHeight * 0.5f);
                    if (lastLabelY == float.MinValue || Math.Abs(labelY - lastLabelY) >= labelMinGap)
                    {
                        string label = BuildWallLabel(isBid ? "BID" : "ASK", price, st);
                        float labelX = panelRight - labelWidth - 8f;
                        var labelRect = new RectangleF(labelX - 4f, labelY, labelWidth + 4f, labelHeight);
                        RenderTarget.FillRectangle(labelRect, _dxHudBg);
                        RenderTarget.DrawRectangle(labelRect, _dxHudBorder, 1f);
                        using (var layout = new TextLayout(NinjaTrader.Core.Globals.DirectWriteFactory, label, _labelFont, labelWidth - 8f, labelHeight))
                            DrawTextLayoutWithOpacity(brush, 1.0f, labelX - 2f, labelY, layout);
                        ...
                        if (ShowStateMarkers)
                        {
                            ...
                            {
                                float markerX = labelX - markerWidth - 4f;
                                var markerRect = new RectangleF(markerX - 3f, labelY, markerWidth, labelHeight);
                                RenderTarget.FillRectangle(markerRect, _dxHudBg);
                                RenderTarget.DrawRectangle(markerRect, _dxHudBorder, 1f);
                                using (var markerLayout = new TextLayout(NinjaTrader.Core.Globals.DirectWriteFactory, marker, _markerFont, markerWidth - 4f, labelHeight))
                                    DrawTextLayoutWithOpacity(brush, 1.0f, markerX, labelY, markerLayout);
                            }
                        }
                ...
                UpdateDecisionCandidates(ref summary, isBid, price, st, referencePrice);
        ...
        private void UpdateDecisionCandidates(ref RenderSummary summary, bool isBid, double price, L2LevelState st, double referencePrice)
        {
            if (referencePrice <= 0 || TickSize <= 0) return;
            double distanceTicks = Math.Abs(price - referencePrice) / TickSize;
            float ranking = st.QualityScore + st.IcebergScore * 0.35f - st.SpoofScore * 0.20f - st.BreakRiskScore * 0.15f;

            if (isBid)
            {
                float current = summary.BestBidQuality + summary.BestBidIceberg * 0.35f - summary.BestBidBreakRisk * 0.15f;
                if (summary.BestBidPrice <= 0 || ranking > current || (Math.Abs(ranking - current) < 0.001f && distanceTicks < summary.BestBidDistanceTicks))
                {
                    summary.BestBidPrice = price;
                    summary.BestBidQuality = st.QualityScore;
                    summary.BestBidIceberg = st.IcebergScore;
                    summary.BestBidBreakRisk = st.BreakRiskScore;
                    summary.BestBidDistanceTicks = distanceTicks;
                }
            }
            else
            {
                float current = summary.BestAskQuality + summary.BestAskIceberg * 0.35f - summary.BestAskBreakRisk * 0.15f;
                if (summary.BestAskPrice <= 0 || ranking > current || (Math.Abs(ranking - current) < 0.001f && distanceTicks < summary.BestAskDistanceTicks))
                {
                    summary.BestAskPrice = price;
                    summary.BestAskQuality = st.QualityScore;
                    summary.BestAskIceberg = st.IcebergScore;
                    summary.BestAskBreakRisk = st.BreakRiskScore;
                    summary.BestAskDistanceTicks = distanceTicks;
                }
            }
        }
        ...
        private void DrawHud(RenderSummary summary)
        {
            ...
            string text = ShowV2Hud
                ? string.Format(
                    "DEPTH RADAR V6 [{0}] {1} | vis B:{2} A:{3} | loaded B:{4} A:{5} | GEN:{6} SPF:{7} ICE:{8} MIG:{9} | Q:{10:0} SPF:{11:0} ICE:{12:0} BRK:{13:0} | reads:{14}",
                ...
                : string.Format(
                    "DEPTH RADAR V6 [JSON] | vis B:{0} A:{1} | loaded B:{2} A:{3} | GEN:{4} SPF:{5} ICE:{6} MIG:{7} | avg conf:{8:0}% | reads:{9}",
                    summary.BidCount, summary.AskCount, totalBids, totalAsks,
                    summary.GenCount, summary.SpoofCount, summary.IceCount, summary.MigCount,
                    avgConfidence, reads);

            float hudFontSize = Math.Max(10f, HudFontSize);
            float w = Math.Min((float)ChartPanel.W - 20f, ShowV2Hud ? 1260f : 920f);
            float h = hudFontSize + 14f;
            ...
            using (var layout = new TextLayout(NinjaTrader.Core.Globals.DirectWriteFactory, text, _hudFont, w - 14f, h))
                RenderTarget.DrawTextLayout(new Vector2(x + 7f, y + 5f), layout, _dxHudText);
        }


        private void DrawDecisionPanel(RenderSummary summary, double referencePrice)
        {
            if (!ShowDecisionPanel || _decisionFont == null || _dxHudBg == null || _dxHudText == null || ChartPanel == null)
                return;

            string action = "NO TRADE";
            string reason = "waiting for stronger depth confirmation";
            string bias = "NEUTRAL";
            bool lowData = Interlocked.Read(ref _jsonReads) <= 0 || (summary.BidCount + summary.AskCount) == 0;
            bool degraded = string.Equals(_sourceQuality, "STALE", StringComparison.OrdinalIgnoreCase) || string.Equals(_sourceQuality, "DEGRADED", StringComparison.OrdinalIgnoreCase);
            bool bidGood = summary.BestBidPrice > 0 && summary.BestBidQuality >= DecisionQualityThreshold && summary.BestBidBreakRisk < BreakRiskThreshold;
            bool askGood = summary.BestAskPrice > 0 && summary.BestAskQuality >= DecisionQualityThreshold && summary.BestAskBreakRisk < BreakRiskThreshold;
            bool bidIce = summary.BestBidIceberg >= IcebergThreshold;
            bool askIce = summary.BestAskIceberg >= IcebergThreshold;
            bool highSpoof = summary.MaxSpoof >= SpoofRiskThreshold;
            bool highBreak = summary.MaxBreakRisk >= BreakRiskThreshold;

            if (lowData || degraded)
            {
                action = "WAIT";
                bias = "LOW DATA";
                reason = degraded ? "source quality degraded/stale" : "not enough live wall data";
            }
            else if (highBreak && !bidGood && !askGood)
            {
                action = "WAIT";
                bias = "BREAK RISK";
                reason = "wall failure risk elevated; do not chase";
            }
            else if (bidGood && !askGood)
            {
                action = "WAIT LONG / BUY PULLBACK";
                bias = "LEAN LONG";
                reason = bidIce ? "bid support + iceberg/reload confirmation" : "best bid support is high quality";
            }
            else if (askGood && !bidGood)
            {
                action = "WAIT SHORT / SELL REJECT";
                bias = "LEAN SHORT";
                reason = askIce ? "ask resistance + iceberg/reload confirmation" : "best ask resistance is high quality";
            }
            else if (bidGood && askGood)
            {
                action = "WAIT";
                bias = "TWO-SIDED";
                reason = "support and resistance both active; wait for gray trigger + break";
            }
            else if (highSpoof)
            {
                action = "WAIT";
                bias = "SPOOF RISK";
                reason = "fleeting/spoof-like liquidity elevated";
            }

            string l1 = string.Format("DEEP6 DECISION V6: {0}  |  {1}", bias, action);
            string l2 = string.Format("REF {0:F2} | BID {1} Q{2:0} I{3:0} BRK{4:0} ({5:0}t) | ASK {6} Q{7:0} I{8:0} BRK{9:0} ({10:0}t)",
                referencePrice,
                summary.BestBidPrice > 0 ? summary.BestBidPrice.ToString("F2", CultureInfo.InvariantCulture) : "--",
                summary.BestBidQuality, summary.BestBidIceberg, summary.BestBidBreakRisk, summary.BestBidDistanceTicks,
                summary.BestAskPrice > 0 ? summary.BestAskPrice.ToString("F2", CultureInfo.InvariantCulture) : "--",
                summary.BestAskQuality, summary.BestAskIceberg, summary.BestAskBreakRisk, summary.BestAskDistanceTicks);
            string l3 = "GRAY: ready for external footprint confirmation | " + reason;

            float decisionFontSize = Math.Max(10f, DecisionFontSize);
            float w = Math.Min(1260f, (float)ChartPanel.W - 20f);
            float h = decisionFontSize * 3f + 26f;
            float x = (float)(ChartPanel.X + ChartPanel.W) - w - 8f;
            float y = (float)ChartPanel.Y + 36f + Math.Max(24f, Math.Max(10f, HudFontSize) + 14f);
            RenderTarget.FillRectangle(new RectangleF(x, y, w, h), _dxHudBg);
            RenderTarget.DrawRectangle(new RectangleF(x, y, w, h), _dxHudBorder, 1f);
            using (var layout = new TextLayout(NinjaTrader.Core.Globals.DirectWriteFactory, l1 + "\n" + l2 + "\n" + l3, _decisionFont, w - 14f, h - 8f))
                RenderTarget.DrawTextLayout(new Vector2(x + 7f, y + 7f), layout, _dxHudText);
        }
        ...
        private void DisposeDx()
        {
            ...
            if (_decisionFont != null) { _decisionFont.Dispose(); _decisionFont = null; }
        ...
        [NinjaScriptProperty]
        [Display(Name = "Color By Intent", Order = 1, GroupName = "Depth Radar V6")]
        public bool ColorByIntent { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Confidence", Order = 2, GroupName = "Depth Radar V6")]
        public bool ShowConfidence { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show State Markers", Order = 3, GroupName = "Depth Radar V6")]
        public bool ShowStateMarkers { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show Quality Scores", Order = 4, GroupName = "Depth Radar V6")]
        public bool ShowQualityScores { get; set; }

        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Min Quality Score", Order = 5, GroupName = "Depth Radar V6",
            Description = "Hide walls below this quality score. Leave 0 to show all walls.")]
        public int MinQualityScore { get; set; }

        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Min Spoof Score", Order = 6, GroupName = "Depth Radar V6",
            Description = "Hide walls below this spoof-like score. Leave 0 to show all walls.")]
        public int MinSpoofScore { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Show V2 HUD", Order = 7, GroupName = "Depth Radar V6")]
        public bool ShowV2Hud { get; set; }


        [NinjaScriptProperty]
        [Range(10, 32)]
        [Display(Name = "Level Label Font Size", Order = 8, GroupName = "Depth Radar V6",
            Description = "Font size for right-side wall/level labels. Increase for large monitors or screenshots.")]
        public int LabelFontSize { get; set; }

        [NinjaScriptProperty]
        [Range(10, 32)]
        [Display(Name = "HUD Font Size", Order = 9, GroupName = "Depth Radar V6",
            Description = "Font size for the top Depth Radar status HUD.")]
        public int HudFontSize { get; set; }

        [NinjaScriptProperty]
        [Range(10, 36)]
        [Display(Name = "Decision Panel Font Size", Order = 10, GroupName = "Depth Radar V6",
            Description = "Font size for the DEEP6 DECISION V6 panel.")]
        public int DecisionFontSize { get; set; }

        [NinjaScriptProperty]
        [Range(240, 600)]
        [Display(Name = "Level Label Panel Width", Order = 11, GroupName = "Depth Radar V6",
            Description = "Reserved right-side width for readable wall labels.")]
        public int LabelPanelWidth { get; set; }


        [NinjaScriptProperty]
        [Display(Name = "Show Decision Panel", Order = 1, GroupName = "Depth Radar V6")]
        public bool ShowDecisionPanel { get; set; }

        [NinjaScriptProperty]
        [Range(0, 1000)]
        [Display(Name = "Max Distance From Price (ticks)", Order = 2, GroupName = "Depth Radar V6",
            Description = "Only render/use walls within this many ticks of live mid/price. 0 disables distance filtering.")]
        public int MaxDistanceFromPriceTicks { get; set; }

        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Decision Quality Threshold", Order = 3, GroupName = "Depth Radar V6")]
        public int DecisionQualityThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Spoof Risk Threshold", Order = 4, GroupName = "Depth Radar V6")]
        public int SpoofRiskThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Iceberg / Reload Threshold", Order = 5, GroupName = "Depth Radar V6")]
        public int IcebergThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "Break Risk Threshold", Order = 6, GroupName = "Depth Radar V6")]
        public int BreakRiskThreshold { get; set; }
