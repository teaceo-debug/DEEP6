...
{
	public class UW_DarkPoolLiquidityMap : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = "Unusual Whales dark-pool / off-lit liquidity map. Plots inferred support, "
					+ "resistance, magnet, absorption and exhaustion zones from dark-pool prints and "
					+ "off/lit volume-by-price. Post-trade inference — not resting orders, not financial advice.";
				Name = "UW_DarkPoolLiquidityMap";
				...
				MaxLevelsToRender = 12;
				MergeNearbyLevels = true;
				MergeDistanceTicks = 4;
		...
		protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
		{
			...
			if (UseSharpDxRendering && snap != null && snap.Levels.Count > 0)
			{
				...
				{
					double hp = (double)l.ClusterWidth / 2.0 * mapScale;
					double pm = (double)l.Price * mapScale;
					if (pm + hp >= minP && pm - hp <= maxP)
						visibleCount++;
				}
				bool labelsAllowed = ShowLabels && LabelMode != LabelDisplayMode.None && visibleCount <= 25;

				foreach (LiquidityLevel lvl in snap.Levels)
				{
					double p = (double)lvl.Price * mapScale;
					double halfW = (double)lvl.ClusterWidth / 2.0 * mapScale;
					if (p + halfW < minP || p - halfW > maxP)
						continue;

					float yTop = chartScale.GetYByValue(p + halfW);
					float yBot = chartScale.GetYByValue(p - halfW);
					if (yBot - yTop < 2f)
					{
						float mid = (yTop + yBot) / 2f;
						yTop = mid - 1f;
						yBot = mid + 1f;
					}
					...
					float conf = (float)Math.Max(0.0, Math.Min(1.0, (double)lvl.Score / 100.0));
					float zoneAlpha = Lerp((float)ZoneOpacityMin, (float)ZoneOpacityMax, conf);
					bool stale = lvl.IsDecayed && FadeStaleLevels;
					if (stale)
						zoneAlpha *= 0.45f;

					float xStart = panelX;
					if (!ExtendLevelsLeft && lvl.FirstSeenUtc != DateTime.MinValue)
						xStart = Math.Max(panelX, chartControl.GetXByTime(lvl.FirstSeenUtc.ToLocalTime()));
					float xEnd = panelX + panelW;
					if (!ExtendLevelsRight && lvl.LastSeenUtc != DateTime.MinValue)
						xEnd = Math.Min(panelX + panelW, chartControl.GetXByTime(lvl.LastSeenUtc.ToLocalTime()));
					if (xEnd - xStart < 8f)
						xEnd = Math.Min(panelX + panelW, xStart + 8f);
					...
					if (ShowLines)
					{
						float lw = Lerp((float)LineWidthMin, (float)LineWidthMax, (float)lvl.PremiumWeight);
						float lineAlpha = Math.Min(0.92f, zoneAlpha * 3f + 0.25f);
						if (lvl.IsFresh)
							lineAlpha = Math.Min(1f, lineAlpha + 0.15f);
						D2D.SolidColorBrush lineBrush = GetDxBrush(new Color4(baseColor.Red, baseColor.Green, baseColor.Blue, lineAlpha));
						...
						// Fresh prints: subtle edge glow
						if (lvl.IsFresh && !stale)
						{
							D2D.SolidColorBrush glow = GetDxBrush(new Color4(baseColor.Red, baseColor.Green, baseColor.Blue, lineAlpha * 0.30f));
							RenderTarget.DrawLine(new Vector2(xStart, yMid - lw - 1f), new Vector2(xEnd, yMid - lw - 1f), glow, 1f);
							RenderTarget.DrawLine(new Vector2(xStart, yMid + lw + 1f), new Vector2(xEnd, yMid + lw + 1f), glow, 1f);
						}
					}

					if (labelsAllowed)
						DrawLevelLabel(lvl, yMid, panelX, panelW, theme, stale, snap.CurrentPrice);

					if ((double)lvl.Score >= HighConfidenceScoreThreshold)
						DrawRightEdgeTag(p, yMid, panelX, panelW, baseColor);
				}
		...
		private void DrawDashboard(LiquiditySnapshot snap, UwTheme theme, float panelX, float panelY, float panelW, float panelH)
		{
			...
			lines.Add("UW Liquidity Map — " + (string.IsNullOrEmpty(resolvedTicker) ? "?" : resolvedTicker));
			...
			float lineH = 15f, pad = 8f, width = 268f;
		...
		private void LogMsg(UwLogLevel level, string msg)
		{
			...
			try
			{
				...
				if (level == UwLogLevel.Error)
					Log("UW_DarkPoolLiquidityMap: " + msg, NinjaTrader.Cbi.LogLevel.Error);
			}
			catch { }
		}
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private UW_DarkPoolLiquidityMap[] cacheUW_DarkPoolLiquidityMap;
		public UW_DarkPoolLiquidityMap UW_DarkPoolLiquidityMap()
		{
			return UW_DarkPoolLiquidityMap(Input);
		}

		public UW_DarkPoolLiquidityMap UW_DarkPoolLiquidityMap(ISeries<double> input)
		{
			if (cacheUW_DarkPoolLiquidityMap != null)
				for (int idx = 0; idx < cacheUW_DarkPoolLiquidityMap.Length; idx++)
					if (cacheUW_DarkPoolLiquidityMap[idx] != null &&  cacheUW_DarkPoolLiquidityMap[idx].EqualsInput(input))
						return cacheUW_DarkPoolLiquidityMap[idx];
			return CacheIndicator<UW_DarkPoolLiquidityMap>(new UW_DarkPoolLiquidityMap(), input, ref cacheUW_DarkPoolLiquidityMap);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.UW_DarkPoolLiquidityMap UW_DarkPoolLiquidityMap()
		{
			return indicator.UW_DarkPoolLiquidityMap(Input);
		}

		public Indicators.UW_DarkPoolLiquidityMap UW_DarkPoolLiquidityMap(ISeries<double> input )
		{
			return indicator.UW_DarkPoolLiquidityMap(input);
		}
	}
