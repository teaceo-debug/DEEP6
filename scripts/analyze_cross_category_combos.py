...
OUT_PATH = OUT_DIR / "cross_category_combo_report.txt"
...
def build_filter_specs() -> list[tuple[str, str, object]]:
    return [
        ("01", "DELT_04 + 60m_extreme", lambda df: df["has_DELT_04"] & df["is_60m_extreme"]),
        ("02", "AUCT_03 + 60m_extreme", lambda df: df["has_AUCT_03"] & df["is_60m_extreme"]),
        ("03", "AUCT_03 + 15m_trend_aligned", lambda df: df["has_AUCT_03"] & df["is_15m_trend_aligned"]),
        ("04", "VOLP_06 + 60m_extreme", lambda df: df["has_VOLP_06"] & df["is_60m_extreme"]),
        ("05", "TRAP_04 + 60m_extreme", lambda df: df["has_TRAP_04"] & df["is_60m_extreme"]),
        ("06", "TRAP_04 + 15m_trend_aligned", lambda df: df["has_TRAP_04"] & df["is_15m_trend_aligned"]),
        ("07", "DELT_04 + AUCT_03", lambda df: df["has_DELT_04"] & df["has_AUCT_03"]),
        ("08", "DELT_04 + TRAP_04", lambda df: df["has_DELT_04"] & df["has_TRAP_04"]),
        ("09", "DELT_04 + EXH_03", lambda df: df["has_DELT_04"] & df["has_EXH_03"]),
        ("10", "AUCT_03 + TRAP_04", lambda df: df["has_AUCT_03"] & df["has_TRAP_04"]),
        ("11", "AUCT_03 + EXH_03", lambda df: df["has_AUCT_03"] & df["has_EXH_03"]),
        ("12", "VOLP_06 + TRAP_04", lambda df: df["has_VOLP_06"] & df["has_TRAP_04"]),
        ("13", "VOLP_06 + EXH_03", lambda df: df["has_VOLP_06"] & df["has_EXH_03"]),
        ("14", "IMB_03 + TRAP_04", lambda df: df["has_IMB_03"] & df["has_TRAP_04"]),
        ("15", "IMB_03 + 60m_extreme", lambda df: df["has_IMB_03"] & df["is_60m_extreme"]),
        ("16", "DELT_04 + TRAP_04 + 60m_extreme", lambda df: df["has_DELT_04"] & df["has_TRAP_04"] & df["is_60m_extreme"]),
        ("17", "DELT_04 + EXH_03 + 60m_extreme", lambda df: df["has_DELT_04"] & df["has_EXH_03"] & df["is_60m_extreme"]),
        ("18", "AUCT_03 + TRAP_04 + 60m_extreme", lambda df: df["has_AUCT_03"] & df["has_TRAP_04"] & df["is_60m_extreme"]),
        ("19", "IMB_03 + TRAP_04 + 60m_extreme", lambda df: df["has_IMB_03"] & df["has_TRAP_04"] & df["is_60m_extreme"]),
        ("20", "DELT_04 + TRAP_04 + 15m_trend_aligned", lambda df: df["has_DELT_04"] & df["has_TRAP_04"] & df["is_15m_trend_aligned"]),
        ("21", "Any TYPE_B bar + 60m_extreme", lambda df: df["has_TYPE_B"] & df["is_60m_extreme"]),
        ("22", "Any TYPE_A bar + 60m_extreme", lambda df: df["has_TYPE_A"] & df["is_60m_extreme"]),
        ("23", "score_final >= 80 + 60m_extreme", lambda df: df["has_score_ge_80"] & df["is_60m_extreme"]),
        ("24", "3+ categories firing same bar + 60m_extreme", lambda df: (df["category_count"] >= 3) & df["is_60m_extreme"]),
        ("25", "4+ signals firing same bar + 60m_extreme", lambda df: (df["signal_count"] >= 4) & df["is_60m_extreme"]),
    ]
...
def main() -> None:
    ...
    lines = [
        "DEEP6 cross-category combo analysis",
        "===================================",
        ...
        "TYPE_A / TYPE_B / score_final >= 80 are presence-based within each grouped observation.",
        "Signal_count uses unique signal_id count. Category_count uses unique category count.",
        ...
        "",
        f"All 25 combos ranked by {FORWARD_WINDOW}-bar average return",
        "--------------------------------------------",
    ]
